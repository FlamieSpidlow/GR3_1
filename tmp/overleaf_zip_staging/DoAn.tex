...
\usepackage{tikz} % Ve so do truc tiep bang LaTeX
...
\usepackage{tabularx}
\usepackage{longtable} % Tạo bảng dài cho danh mục viết tắt và thuật ngữ
\usepackage{booktabs}
\usepackage{ragged2e}
\usepackage{makecell}
...
\newif\ifshowtodos
\showtodosfalse
\newcommand{\thesistodo}[1]{\ifshowtodos\par\noindent\textbf{[TODO: #1]}\par\fi}
\newcommand{\classname}[1]{\mbox{\texttt{#1}}}

% TODO hinh/so do can bo sung sau:
% - Use case tong quat.
% - Use case phan ra nhom tai khoan/chu de/gameplay/tien do.
% - Activity diagram quy trinh choi.
% - So do kien truc tong the.
% - So do goi/module.
% - Class diagram muc cao.
% - Sequence diagram chon man va sinh bang.
% - Sequence diagram nhap dap an.
% - Anh man hinh menu chinh.
% - Anh chon tai khoan.
% - Anh chon chu de.
% - Anh chon man.
% - Anh man choi.
...
\usepackage[backend=biber,style=ieee]{biblatex}  % backend=biber is better for biblatex
...
% Cau hinh hien thi ma nguon co the bo sung sau neu can.

\newcommand{\codeword}[1]{\texttt{\seqsplit{#1}}}
...
\def \TITLE{THIẾT KẾ VÀ XÂY DỰNG TRÒ CHƠI GIẢI ĐỐ TỪ VỰNG TIẾNG VIỆT THEO CHỦ ĐỀ TRÊN NỀN TẢNG GODOT}
\def \AUTHOR{NGUYỄN MINH HIẾU}
\def \STUDENTID{202225626}
...
\newpage
%\pagestyle{fancy} % Áp dụng header và footer
\chapter[PHÂN TÍCH, THIẾT KẾ VÀ ĐÁNH GIÁ HỆ THỐNG]{PHÂN TÍCH, THIẾT KẾ VÀ ĐÁNH GIÁ HỆ THỐNG}
...
\newpage
%\pagestyle{fancy} % Áp dụng header và footer
% Phần hướng dẫn của template, không đưa vào bản báo cáo chính.
...
% Bật lại khối phụ lục nếu cần bổ sung tài liệu kỹ thuật, bảng kiểm thử chi tiết
% hoặc hướng dẫn sử dụng ứng dụng.
% \appendixpage
% \appendices
% \addappheadtotoc
