...
Nội dung chương tập trung vào hệ thống đã triển khai trong project Godot hiện tại. Phần kiểm thử trình bày kết quả thực tế: các trường hợp kiểm thử chức năng được chạy bằng kịch bản kiểm thử tự động trên Godot 4.6.3 và kiểm thử giao diện được đối chiếu trên ảnh chụp màn hình thật.
...
\begin{figure}[H]
\centering
\resizebox{0.95\textwidth}{!}{\input{Hinhve/chuong4/kien_truc_he_thong.tex}}
...
\begin{figure}[H]
\centering
\resizebox{0.92\textwidth}{!}{\input{Hinhve/chuong4/luong_chuyen_man.tex}}
...
\begin{figure}[H]
\centering
\resizebox{0.90\textwidth}{!}{\input{Hinhve/chuong4/class_diagram_rut_gon.tex}}
...
\begin{figure}[H]
\centering
\resizebox{0.95\textwidth}{!}{\input{Hinhve/chuong4/sequence_bat_dau_man.tex}}
...
\begin{figure}[H]
\centering
\resizebox{0.95\textwidth}{!}{\input{Hinhve/chuong4/sequence_nhap_dap_an.tex}}
...
\begin{figure}[H]
\centering
\resizebox{0.80\textwidth}{!}{\input{Hinhve/chuong4/cau_truc_json_luu_tru.tex}}
...
Các hình dưới đây là ảnh chụp màn hình thực tế của ứng dụng khi chạy trên Godot 4.6.3 ở độ phân giải 1280$\times$720. Hình~\ref{fig:screen-menu} là menu chính với phần chọn tài khoản. Hình~\ref{fig:screen-select} là luồng chọn chủ đề và chọn màn. Hình~\ref{fig:screen-board} là màn chơi chính, gồm vùng bảng ô chữ bên trái và vùng gợi ý, nhập đáp án, sao, điểm bên phải. Hình~\ref{fig:screen-play} cho thấy trạng thái khi đã giải được một phần các từ, và Hình~\ref{fig:screen-result} là hộp thoại kết quả sau khi hoàn thành màn.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{Hinhve/screenshots/01_menu.png}
\caption{Màn hình menu chính}
\label{fig:screen-menu}
\end{figure}
...
\centering
\includegraphics[width=\textwidth]{Hinhve/screenshots/02_chon_chu_de.png}
\caption{Chọn chủ đề}
\end{subfigure}
\hfill
\begin{subfigure}{0.49\textwidth}
\centering
\includegraphics[width=\textwidth]{Hinhve/screenshots/03_chon_man.png}
\caption{Chọn màn chơi}
\end{subfigure}
\caption{Luồng chọn chủ đề và chọn màn chơi}
\label{fig:screen-select}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{Hinhve/screenshots/04_man_choi.png}
\caption{Màn chơi chính khi mới bắt đầu}
\label{fig:screen-board}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{Hinhve/screenshots/05_dang_choi.png}
\caption{Màn chơi khi đã giải được một phần các từ}
\label{fig:screen-play}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{Hinhve/screenshots/06_ket_qua.png}
\caption{Hộp thoại kết quả sau khi hoàn thành màn}
\label{fig:screen-result}
\end{figure}
...
Mã nguồn logic & 13 file \code{.gd}, 4945 dòng & Thống kê từ thư mục \code{scripts}. Không tính file \code{.uid}. \\ \hline
Scene & 4 file \code{.tscn}, 471 dòng & Gồm \code{Main.tscn}, \code{StartMenu.tscn}, \code{GameBoard.tscn}, \code{Tile.tscn}. \\ \hline
...
Kết quả xây dựng nguyên mẫu gồm luồng chọn tài khoản, chọn chủ đề, chọn màn, sinh bảng ô chữ, chọn gợi ý, nhập đáp án, dùng sao để mở gợi ý, tính điểm, tính nhiệm vụ sao và ghi nhận tiến độ theo tài khoản. Các chức năng này được triển khai cục bộ, chưa phụ thuộc vào máy chủ hoặc dịch vụ trực tuyến. Về hiệu năng sinh nội dung, bộ sinh bảng tạo được 84/84 màn hợp lệ cho 28 chủ đề với thời gian trung bình khoảng 3,1\,ms mỗi màn; số liệu chi tiết được trình bày trong Mục~\ref{subsection:5.2.1}.

\section{Kiểm thử}
\label{section:4.7}

Phần này trình bày kết quả kiểm thử nguyên mẫu. Các trường hợp kiểm thử chức năng được thực hiện bằng một kịch bản kiểm thử tự động chạy ở chế độ không giao diện (\code{--headless}) trên Godot 4.6.3. Kịch bản gọi trực tiếp vào các lớp logic của trò chơi (\code{SaveManager}, \code{ThemedLevelProvider}, \code{BoardGenerator}, \code{VietnameseNormalizer}, \code{GameState}) và so sánh kết quả thực tế với kết quả mong đợi. Toàn bộ 14/14 trường hợp kiểm thử chức năng đều đạt. Kiểm thử giao diện được đối chiếu trực tiếp trên ảnh chụp màn hình ở Mục~\ref{section:4.5}.
...
\caption{Kết quả kiểm thử chức năng (14/14 đạt)}
...
\textbf{Mã} & \textbf{Chức năng} & \textbf{Dữ liệu/thao tác kiểm thử} & \textbf{Kết quả thực tế} \\ \hline
\endhead
TC01 & Tạo tài khoản & Nhập tên tài khoản mới và xác nhận tạo. Kiểm tra tài khoản xuất hiện trong danh sách và được chọn làm hồ sơ hiện tại. & Đạt. Tài khoản mới được tạo và tự động đặt làm hồ sơ hiện tại. \\ \hline
TC02 & Chọn tài khoản & Chọn một tài khoản đã tồn tại và xem tiến độ tương ứng ở danh sách chủ đề/màn. & Đạt. Hồ sơ hiện tại chuyển đúng sang tài khoản được chọn. \\ \hline
TC03 & Chọn chủ đề & Chọn một chủ đề trong menu và mở danh sách màn của chủ đề đó. & Đạt. Mở đúng danh sách màn (ví dụ chủ đề Phòng ngủ có 3 màn). \\ \hline
TC04 & Chọn màn chơi & Chọn một màn trong chủ đề, kiểm tra hệ thống chuyển sang màn chơi và hiển thị bảng/gợi ý. & Đạt. Màn chơi được khởi tạo và sinh bảng (màn 1 có 9 từ). \\ \hline
TC05 & Sinh bảng ô chữ & Dùng danh sách từ của một chủ đề để sinh bảng; kiểm tra số từ được đặt và tình trạng bảng rời rạc. & Đạt. Bảng gồm 9 từ, tất cả đều có giao điểm, không có từ cô lập. \\ \hline
TC06 & Nhập đáp án đúng & Chọn gợi ý, nhập đáp án đúng có dấu tiếng Việt hoặc có khoảng trắng tương ứng. & Đạt. Khớp đáp án khi nhập hoa/thường, có/không khoảng trắng và có dấu. \\ \hline
TC07 & Nhập đáp án sai/rỗng & Chọn gợi ý rồi nhập sai hoặc để trống ô nhập. & Đạt. Đáp án thiếu dấu, để trống hoặc sai đều bị từ chối. \\ \hline
TC08 & Dùng gợi ý mở chữ & Chọn một từ chưa hoàn thành, dùng sao để mở một chữ. & Đạt. Mở đúng một ký tự và ghi nhận số chữ đã gợi ý. \\ \hline
TC09 & Dùng gợi ý mở cả từ & Chọn một từ chưa hoàn thành, dùng sao để mở toàn bộ từ. & Đạt. Toàn bộ từ được mở và đánh dấu hoàn thành. \\ \hline
TC10 & Không đủ sao & Đưa tài khoản về trạng thái không đủ sao rồi thử dùng gợi ý. & Đạt. Khi hết sao, thao tác dùng gợi ý bị từ chối. \\ \hline
TC11 & Tính điểm và nhiệm vụ & Hoàn thành một số từ, kiểm tra điểm, chuỗi đúng liên tiếp, nhiệm vụ và số sao nhiệm vụ. & Đạt. Cộng điểm theo từ (ví dụ +1600) và sinh đủ 3 nhiệm vụ. \\ \hline
TC12 & Lưu tiến độ & Hoàn thành màn, quay về menu và kiểm tra trạng thái sao/hoàn thành của màn. & Đạt. Ghi nhận trạng thái hoàn thành, số sao và điểm của màn. \\ \hline
TC13 & Tải lại tiến độ & Đóng và mở lại game sau khi đã hoàn thành màn. & Đạt. Đọc lại đúng tiến độ đã lưu (điểm tốt nhất 1500). \\ \hline
TC14 & Dữ liệu lưu không hợp lệ & Thử với tệp lưu thiếu trường hoặc cấu trúc cũ để kiểm tra cơ chế chuẩn hóa. & Đạt. Tệp lưu thiếu trường được chuẩn hóa về cấu trúc hợp lệ, không gây lỗi. \\ \hline
\end{longtable}
...
Menu và tài khoản & Người chơi nhận biết được tài khoản hiện tại, biết cách thêm/chọn/xóa tài khoản. & Đạt. Menu hiển thị rõ tài khoản hiện tại và nút quản lý (Hình~\ref{fig:screen-menu}). \\ \hline
Chủ đề và màn chơi & Danh sách chủ đề và màn đủ rõ, không làm người chơi nhầm giữa chọn chủ đề và chọn màn. & Đạt. Bước chọn chủ đề và chọn màn tách riêng (Hình~\ref{fig:screen-select}). \\ \hline
Bảng ô chữ & Ô chữ, gợi ý, vùng nhập và nút thao tác không chồng nhau ở kích thước cửa sổ phổ biến. & Đạt. Bảng, gợi ý, ô nhập và nút không chồng nhau (Hình~\ref{fig:screen-board}). \\ \hline
Phản hồi thao tác & Thông báo đúng, sai, thiếu sao, hoàn thành màn hiển thị đủ rõ và không che thao tác chính. & Đạt. Đánh dấu từ hoàn thành và hộp thoại kết quả hiển thị rõ (Hình~\ref{fig:screen-play}, \ref{fig:screen-result}). \\ \hline
\end{tabularx}
\caption{Kết quả kiểm thử giao diện (quan sát ở độ phân giải 1280$\times$720)}
...
Số người chơi thử & Ghi số người tham gia trong đợt thử nghiệm. & Chưa thực hiện. \\ \hline
Số màn thử & Ghi số màn được dùng trong phiên thử. & Chưa thực hiện. \\ \hline
Số màn hoàn thành & Ghi số màn người chơi hoàn thành. & Chưa thực hiện. \\ \hline
Thời gian hoàn thành trung bình & Tính từ thời gian hoàn thành từng lượt chơi. & Chưa thực hiện. \\ \hline
Số lần dùng gợi ý trung bình & Ghi số lần mở chữ hoặc mở cả từ. & Chưa thực hiện. \\ \hline
Nhận xét giao diện & Tổng hợp lỗi khó thao tác, khó đọc hoặc phản hồi chưa rõ. & Chưa thực hiện. \\ \hline
\end{tabularx}
...
Mặc dù toàn bộ kiểm thử chức năng đã đạt và kiểm thử giao diện đã được đối chiếu trên ảnh chụp, các kết quả này chưa thay thế cho một đợt kiểm thử đầy đủ trên nhiều thiết bị và nhiều nhóm người dùng. Kiểm thử chức năng được thực hiện ở mức logic thông qua kịch bản tự động, chưa bao phủ toàn bộ thao tác chuột/bàn phím thực tế. Kiểm thử giao diện mới được quan sát ở độ phân giải 1280$\times$720, chưa kiểm tra đầy đủ ở nhiều độ phân giải khác. Thử nghiệm chơi với người dùng thật chưa được thực hiện. Một số nội dung như độ khó từ vựng, mức cân bằng sao/gợi ý, khả năng đọc giao diện ở nhiều độ phân giải và hành vi nhập tiếng Việt trên môi trường web cần được kiểm tra thêm. Vì vậy, phần kiểm thử trong chương này xác nhận tính đúng đắn chức năng ở mức nguyên mẫu, nhưng chưa đưa ra kết luận về hiệu quả học tập.
